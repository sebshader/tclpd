...
proc+ bitmap::0_config2 {self args} {
    uplevel 1 "bitmap::0_config $self [string map {@ $} $args]"
}
...
proc+ bitmap::0_getcell {self args} {
    ...
    set c [pd::arg 1 int]
    set w [dict get $@config -uwidth]
    set r [lindex $@data [expr {$r*$w+$c}]]
    pd::outlet $self 0 float $r
    if {$@send ne {}} {
        pd::send $@send float [list [list float $r]]
    }
}
...
proc+ bitmap::save {self args} {
    ...
    if {$@asend eq {}} {
    		set @asend [dict get $c2 -sendsymbol]
    		dict set c -sendsymbol $@asend
    }
    if {$@arecv eq {}} {
    		set @arecv [dict get $c2 -receivesymbol]
    		dict set c -receivesymbol $@arecv
    }
    dict set c -sendsymbol $@asend
	dict set c -receivesymbol $@arecv
    return [list #X obj $@x $@y bitmap {*}[pd::add_empty $c] \; \
        \#bitmap setdata {*}$@data \; ]
}
...
proc+ bitmap::properties {self args} {
    ...
    dict set c -sendsymbol $@asend
	dict set c -receivesymbol $@arecv
    set buf [list propertieswindow %s $c $title]\n
