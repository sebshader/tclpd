...
namespace eval ::pd {
    ...
    proc sendbang {rec} {
    	if {[llength $rec] eq 2} {
    		set rec [lindex $rec 1]
    	}
    	pd_send $rec bang {{null 0}}
    }
    
    proc send {rec {sel {}} {args {}}} {
    	if {[llength $rec] eq 2} {
    		set rec [lindex $rec 1]
    	}
    	if {[llength $sel] eq 2} {
    		set sel [lindex $sel 1]
    	}
    	set args [lindex $args 0]
    	if {[expr {$args eq {}}] && [expr {$sel ne "bang"}]} {
    		if {$sel eq {}} {
    			set sel bang
    		} elseif {[string is double $sel]} {
    			set args [list [list float $sel]]
    			set sel float
    		} else {
    			set args [list [list symbol $sel]]
    			set sel symbol
    		}
    	}
